...
from ..utils import spatial_filter, spatial_resize, RESIZE_METHODS, WINDOW_METHODS
...
class QuantitativeDataset:
    ...
    def _evaluate_baseline_signal(self, TR: Optional[float] = None) -> torch.Tensor:
        ...
        if self.T1 is not None and TR is not None:
            x = x * (1 - torch.exp(-TR / self.T1[None,]))
        return x

    def _apply_b0(self, x: torch.Tensor, tt: torch.Tensor) -> torch.Tensor:
        if self.B0 is not None:
            x = x.to(torch.complex64)
            x = x * torch.exp(-1j * 2 * torch.pi * self.B0[None,] * tt)
        return x
    
    def _evaluate_gre(self, tt: torch.Tensor, TR: Optional[float] = None) -> torch.Tensor:
        """
        ...
        x = x * torch.exp(-tt / self.T2s[None,]).to(x.dtype)
        x = self._apply_b0(x, tt)
        return x

    def _evaluate_se(self, tt: torch.Tensor, TE: float, TR: Optional[float] = None) -> torch.Tensor:
        """
        ...
        if gre_inds.any():
            x[gre_inds] = x[gre_inds] * torch.exp(-tt[gre_inds] / self.T2s[None,]).to(x.dtype)
            x[gre_inds] = self._apply_b0(x[gre_inds], tt[gre_inds])
        if se_inds.any():
            tt_se = tt[se_inds]
            x[se_inds] = x[se_inds] * torch.exp(-((tt_se - TE).abs() * T2pinv[None,]) - (tt_se / self.T2[None,])).to(x.dtype)
    ...
    def evaluate(self, 
        ...
        if signal_type == "se":
            assert TE is not None, "TE must be provided for Spin Echo signal evaluation."
        ...
        if apply_maps:
            assert self.mps is not None, "sensitivity maps must be provided to apply maps."
            x = x[:, None] * self.mps[None,]
    ...
    def __validate_tensor(self, x, name):
        if x is not None:
            assert x.shape[-self.ndim:] == self.im_size, \
...
def get_hf_dataset(dataset: SimDataset, 
                   device: Optional[torch.DeviceObjType] = None,
                   verbose: bool = False) -> Dict[str, torch.Tensor]:
    assert isinstance(dataset, SimDataset), "dataset must be an instance of SimDataset Enum."
    ...
    if os.path.exists(file_path):
        ...
        if verbose:
            print(f"Loading {dataset_str} from Hugging Face Hub: {paths.HF_REPO}/{file_name}")
        snapshot_download(
...
def load_dataset(
    ...
    if im_size is not None:
        ...
        assert len(im_size_in) == len(im_size), \
            f"Mismatch in ndim of data ({len(im_size_in)}) and im_size arg ({len(im_size)})."
        if verbose:
            print(f"Resizing dataset from {im_size_in} to {im_size} using `{resize_method}` method.")
