"""
Code from Patrick kidger github: https://github.com/patrick-kidger/torchcubicspline

For faster cubic interpolation on GPU, which I adapted for complex data.
"""
import math
...
def _validate_input(t, X):
    ...
    if len(t.shape) != 1:
        raise ValueError(
            "t must be one dimensional. It instead has shape {}.".format(tuple(t.shape))
        )
    prev_t_i = -math.inf
    for t_i in t:
        if t_i <= prev_t_i:
...
class NaturalCubicSpline:
    ...
    def derivative(self, t, order=1):
        ...
        if order == 1:
            ...
            deriv = (
                2 * self._c[..., index, :]
                + 6 * self._d[..., index, :] * fractional_part
            )
        else:
...
def _torch_cubic_interpolate(
    ...
    Wrapper on floats
    """
    coeffs = natural_cubic_spline_coeffs(xi, yi)
    spline = NaturalCubicSpline(coeffs)
    return spline.evaluate(xo)


def torch_cubic_interpolate(
    xi: torch.Tensor,
    yi: torch.Tensor,
    xo: torch.Tensor,
) -> torch.Tensor:
    """
    Interpolates values yi defined at points xi to new points xo using cubic interpolation.

    Parameters
    ----------
    xi : torch.Tensor
        1-D tensor of shape (N,) of original points where values yi are defined.
    yi : torch.Tensor
        N-D tensor of shape (..., N, C), where C is a batch dimension.
    xo : torch.Tensor
        1-D tensor of shape (M,) of new points where we want to interpolate values.

    Returns
    -------
    torch.Tensor
        N-D tensor of shape (..., M, C) with interpolated values at points xo.
    """

    if torch.is_complex(yi):
        real_part = _torch_cubic_interpolate(xi, yi.real, xo)
        imag_part = _torch_cubic_interpolate(xi, yi.imag, xo)
        return real_part + 1j * imag_part
    else:
        return _torch_cubic_interpolate(xi, yi, xo)
